...
// Map VeraScann's forensic_signals strings onto our 6 fixed signal slots
function mapSignals(forensicSignals = [], verdict) {
  const isBad = verdict !== "GENUINE";
  return SIGNALS.map((name) => {
    const hit = forensicSignals.find(s =>
      s.toLowerCase().split(" ").some(w => name.toLowerCase().includes(w) && w.length > 3)
    );
    return {
      name,
      status: hit ? (isBad ? "FAIL" : "PASS") : "PASS",
      detail: hit || null,
    };
  });
}
...
export default function ScanPage() {
  ...
  const [uploadPreview, setUploadPreview] = useState(null);
  const [uploadFile,    setUploadFile]    = useState(null); // raw File — sent directly as multipart
  ...
  // ── Core scan: posts a Blob/File directly to VeraScann as multipart ──────
  const runScan = useCallback(async (blob) => {
    setPhase("scanning");
    setProgress(0);
    setActiveSig(0);
    ...
    try {
      ...
      const verdict =
        data.verdict === "GENUINE"    ? "GENUINE"    :
        data.verdict === "SUSPICIOUS" ? "SUSPICIOUS" : "COUNTERFEIT";

      setTimeout(() => {
        setResult({
          verdict,
          confidence:       Math.round(data.confidence ?? 0),
          genuine_prob:     data.genuine_probability ?? 0,
          fake_prob:        data.fake_probability    ?? 0,
          product:          data.product             || product.label,
          recommendation:   data.recommendation      || null,
          forensic_signals: data.forensic_signals    || [],
          signals:          mapSignals(data.forensic_signals || [], verdict),
        });
        setPhase("result");
      }, 400);
  ...
  // ── Trigger scan from whichever input mode is active ─────────────────────
  const captureAndScan = useCallback(async () => {
    if (inputMode === "upload" && uploadFile) {
      runScan(uploadFile);
      return;
    }
    if (inputMode === "camera" && cameraOn && videoRef.current && canvasRef.current) {
      const video  = videoRef.current;
      const canvas = canvasRef.current;
      canvas.width  = video.videoWidth  || 640;
      canvas.height = video.videoHeight || 480;
    ...
    setError("Please upload a product image or enable your camera.");
  }, [inputMode, uploadFile, cameraOn, runScan]);

  const reset = () => {
    setPhase("idle"); setResult(null); setError(null);
    setProgress(0);   setActiveSig(-1);
  ...
  const verdictColor = result ? (VERDICT_COLOR[result.verdict] || "#999") : "#999";
  const canScan      = inputMode === "upload" ? !!uploadFile : cameraOn;
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <div style={{ background: "#141414", border: "1px solid #2A2A2A", borderRadius: 12, overflow: "hidden" }}>
          ...
          <div style={{ background: "#000", height: 280, display: "flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden" }}>
            ...
            <video ref={videoRef} autoPlay playsInline muted
              style={{ width: "100%", height: "100%", objectFit: "cover", display: cameraOn && inputMode === "camera" && phase === "idle" ? "block" : "none" }} />
            <canvas ref={canvasRef} style={{ display: "none" }} />

            {/* Upload preview — shown while idle or result */}
            {inputMode === "upload" && uploadPreview && (
              <img src={uploadPreview} alt="preview"
                style={{ width: "100%", height: "100%", objectFit: "contain", opacity: phase === "scanning" ? 0.3 : 1, transition: "opacity 0.3s" }} />
            )}

            {/* Idle — camera mode, no stream */}
            {phase === "idle" && inputMode === "camera" && !cameraOn && (
              <div style={{ width: 130, height: 130, border: "2px solid #00C9A7", borderRadius: 10, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8 }}>
                <div style={{ fontSize: 36 }}>📷</div>
                <div style={{ fontSize: 10, color: "#555", textAlign: "center", lineHeight: 1.5 }}>Enable camera or<br/>switch to Upload</div>
              </div>
            )}
            ...
            {phase === "idle" && inputMode === "upload" && !uploadPreview && (
              <div onClick={() => fileRef.current?.click()}
                style={{ width: 160, height: 140, border: "2px dashed #2A2A2A", borderRadius: 10, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8, cursor: "pointer" }}
                onMouseEnter={e => e.currentTarget.style.borderColor = "#00C9A7"}
                onMouseLeave={e => e.currentTarget.style.borderColor = "#2A2A2A"}>
                <div style={{ fontSize: 36 }}>📁</div>
                <div style={{ fontSize: 10, color: "#555", textAlign: "center", lineHeight: 1.5 }}>Tap to upload<br/>product photo</div>
            ...
            {/* Result overlay at bottom of viewport */}
            {phase === "result" && result && (
              <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: "rgba(0,0,0,0.78)", backdropFilter: "blur(4px)", padding: "14px 16px", display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ width: 48, height: 48, borderRadius: "50%", background: `${verdictColor}18`, border: `2px solid ${verdictColor}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, flexShrink: 0 }}>
                  {result.verdict === "GENUINE" ? "✓" : result.verdict === "COUNTERFEIT" ? "✗" : "⚠"}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 17, fontWeight: 700, color: verdictColor }}>{result.verdict}</div>
                  <div style={{ fontSize: 11, color: "#888", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {result.product} · {result.confidence}% confidence
                  </div>
                </div>
              </div>
            )}

            {cameraOn && inputMode === "camera" && phase === "idle" && (
              <div style={{ position: "absolute", top: 10, left: 10, fontSize: 10, color: "#00C9A7", background: "rgba(0,201,167,0.1)", padding: "3px 8px", borderRadius: 4, border: "1px solid rgba(0,201,167,0.2)" }}>● LIVE</div>
            )}
            {inputMode === "upload" && uploadPreview && phase === "idle" && (
              <div style={{ position: "absolute", top: 10, left: 10, fontSize: 10, color: "#00C9A7", background: "rgba(0,201,167,0.1)", padding: "3px 8px", borderRadius: 4, border: "1px solid rgba(0,201,167,0.2)" }}>✓ Image ready</div>
          ...
          <div style={{ padding: "14px 18px" }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {inputMode === "camera" ? (
                ...
                phase !== "result" && (
                  <button onClick={() => fileRef.current?.click()} style={{ flex: 1, background: "#1A1A1A", color: "#999", border: "1px solid #2A2A2A", borderRadius: 8, padding: 10, fontSize: 12, cursor: "pointer" }}>
                    {uploadFile ? "Change Photo" : "Choose Photo"}
                  </button>
                )
              )}

              {phase === "idle" && (
                <button onClick={captureAndScan} disabled={!canScan}
                  style={{ flex: 1, background: canScan ? "#00C9A7" : "#1A1A1A", color: canScan ? "#000" : "#444", border: canScan ? "none" : "1px solid #2A2A2A", borderRadius: 8, padding: 10, fontSize: 13, fontWeight: 700, cursor: canScan ? "pointer" : "not-allowed" }}>
                  {inputMode === "upload"
                    ? (uploadFile ? "Analyse Photo →" : "Upload photo first")
                    : (cameraOn  ? "Scan Product"    : "Enable Camera")}
                </button>
              )}
              {phase === "scanning" && (
                <button disabled style={{ flex: 1, background: "#0D2E24", color: "#00C9A7", border: "1px solid #009B82", borderRadius: 8, padding: 10, fontSize: 13 }}>Analysing...</button>
              )}
              {phase === "result" && (
                <button onClick={reset} style={{ flex: 1, background: "#1A1A1A", color: "#666", border: "1px solid #2A2A2A", borderRadius: 8, padding: 10, fontSize: 13, cursor: "pointer" }}>Scan Again</button>
              )}
            </div>
          </div>
        </div>
        ...
        <div style={{ background: "#141414", border: "1px solid #2A2A2A", borderRadius: 12, padding: "16px 18px" }}>
          ...
          {(result?.signals || SIGNALS.map(name => ({ name, status: null }))).map((sig, i) => {
            ...
            return (
              <div key={i} style={{ display: "flex", gap: 10, padding: "9px 0", borderBottom: "1px solid #1A1A1A", opacity: phase === "idle" ? 0.45 : 1 }}>
                ...
                {isDone && sig.status && (
                  <div style={{ fontSize: 11, fontWeight: 700, color: STATUS_COLOR[sig.status] || "#555", flexShrink: 0 }}>{sig.status}</div>
                )}
              </div>
            );
          })}
          ...
          {phase === "result" && result && (
            <div style={{ display: "flex", gap: 10, marginTop: 14 }}>
              {[["Genuine", result.genuine_prob, "#43A047"], ["Fake", result.fake_prob, "#E53935"]].map(([label, val, color]) => (
                <div key={label} style={{ flex: 1, background: "#111", border: "1px solid #1A1A1A", borderRadius: 8, padding: "10px 12px" }}>
                  <div style={{ fontSize: 10, color: "#555", marginBottom: 4 }}>{label}</div>
                  <div style={{ fontSize: 15, fontWeight: 700, color }}>{Number(val).toFixed(1)}%</div>
                  <div style={{ height: 3, background: "#1A1A1A", borderRadius: 2, marginTop: 6 }}>
                    <div style={{ height: 3, width: `${val}%`, background: color, borderRadius: 2, transition: "width 0.6s ease" }} />
                  </div>
                </div>
          ...
          {/* AI-identified product (if different from selected) */}
          {phase === "result" && result?.product && result.product !== product.label && (
            <div style={{ marginTop: 10, padding: "8px 10px", background: "rgba(96,165,250,0.06)", border: "1px solid rgba(96,165,250,0.15)", borderRadius: 6 }}>
              <div style={{ fontSize: 10, color: "#60A5FA", fontWeight: 600, marginBottom: 2 }}>AI identified product</div>
              <div style={{ fontSize: 11, color: "#888" }}>{result.product}</div>
            </div>
          )}
          ...
          {/* Recommendation */}
          {phase === "result" && result?.recommendation && (
            <div style={{ marginTop: 10, padding: "8px 10px", background: "rgba(0,201,167,0.05)", border: "1px solid rgba(0,201,167,0.12)", borderRadius: 6 }}>
              <span style={{ fontSize: 11, color: "#00C9A7", fontWeight: 600 }}>Recommendation: </span>
              <span style={{ fontSize: 11, color: "#888" }}>{result.recommendation}</span>
            </div>
          )}
          ...
          {phase === "result" && result?.verdict === "COUNTERFEIT" && (
            <div style={{ marginTop: 14, background: "rgba(0,201,167,0.06)", border: "1px solid rgba(0,201,167,0.15)", borderRadius: 8, padding: "12px 14px" }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: "#00C9A7", marginBottom: 4 }}>Verified Vendors Nearby</div>
              <div style={{ fontSize: 11, color: "#666", lineHeight: 1.6, marginBottom: 8 }}>Pharmacies within 1.2km carry genuine {product.label}. Pay via Squad, get a receipt code.</div>
              <button onClick={() => navigate(`/vendor/locator?product=${product.label}&city=Lagos`)}
                style={{ background: "#00C9A7", color: "#000", border: "none", borderRadius: 6, padding: "8px 16px", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
                Find Verified Vendor →
              </button>
            </div>
          ...
          {phase === "result" && result?.verdict === "SUSPICIOUS" && (
            <div style={{ marginTop: 14, background: "rgba(245,166,35,0.06)", border: "1px solid rgba(245,166,35,0.2)", borderRadius: 8, padding: "12px 14px" }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: "#F5A623", marginBottom: 4 }}>⚠ Proceed with Caution</div>
              <div style={{ fontSize: 11, color: "#666", marginBottom: 8 }}>Anomalies detected in the packaging. Purchase only from a verified vendor.</div>
              <button onClick={() => navigate(`/vendor/locator?product=${product.label}&city=Lagos`)}
                style={{ background: "#F5A623", color: "#000", border: "none", borderRadius: 6, padding: "8px 16px", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
                Find Verified Vendor →
              </button>
            </div>
          )}
